...
def load_data(domain_path, domain_dataset, window_size=10, batch_size=128, feature_cols=None):
    folder_key, domain_dataset = domain_dataset  # unpack (folder_path, files)
...
def create_domains(domains_path):
    ...
    domain_label_map = {
        row["Domain Name"]: f"{row['Attack Type']}_{row['Node']}_{row['Version']}"
        for _, row in df.iterrows()
        if "failing" not in row["Attack Type"].lower()
    }

    domains = {}
    for attack_type in os.listdir(domains_path):
        if attack_type == "failing_node":
            continue
        ...
        for domain in os.listdir(attack_path):
            ...
            label = domain_label_map.get(domain, folder_key)
